...
def main(bf16=False):
    ...
    dest=ROOT/('build/rtl/bf16-spm-network' if bf16 else 'build/rtl/spm-network');dest.mkdir(parents=True,exist_ok=True)
    run(['sbt',f'runMain zircon.GenerateSPMExample {dest}'+(' bf16' if bf16 else '')],dest/'generate.log',ROOT/'hardware')
    ...
    width=16 if bf16 else 32;word=width//8;mask=(1<<word)-1
    a=SPM(256,data_width=width);b=SPM(256,data_width=width);mul=BF16Fma() if bf16 else INT32Mul();rows=[];expect=[]
    net=Network().add('a',a).add('mul',mul).add('b',b).connect('a','mul',b=0x4000 if bf16 else 3,c=0x3f80 if bf16 else 0)
    ...
    def cycle(mode,request=None,ready=True,reset=False,flush=False):
        ...
        rows.append(' '.join(map(str,[int(reset),int(flush),mode,int(request is not None),r.address,int(r.write),r.data,mask if r.mask is None else r.mask,r.tag,int(ready)]))+'\n')
        ...
        return inp
    cycle(0,reset=True)
    for adr in range(0,256,word):cycle(0,MemoryRequest(adr,True,0x3e00+adr if bf16 else adr+5))
    for _ in range(10):cycle(0)
    rng=np.random.default_rng(4921);held=None
    for k in range(20000):
        if held is None and rng.random()>.15:
            adr=int(rng.integers(256//word))*word;held=MemoryRequest(adr,tag=adr)
    ...
    for adr in range(0,256,word):cycle(2,MemoryRequest(adr))
    ...
    (dest/'alignment.json').write_text(json.dumps(dict(implementation_hash=implementation,arithmetic_contract_hash=contract_hash(),cycles=len(rows),seed=4921,discrepancy_cycles=0,rtl_sha256=hashlib.sha256((dest/'Unit.sv').read_bytes()).hexdigest(),contract_hash=spm_contract_hash(),verilator=ver),indent=2))
    print('PASS SPM -> '+('BF16Fma' if bf16 else 'INT32Mul')+' -> SPM',len(rows),'cycles')

if __name__=='__main__':main('--bf16' in sys.argv)
