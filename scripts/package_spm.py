"""Create the local v0.2.0 SPM RTL/evidence bundle after both profiles qualify."""
...
def package():
    ...
    target=ROOT/'dist/zircon-asic-0.2.0-spm-evidence.zip';target.parent.mkdir(exist_ok=True)
    ...
    for suffix in ('.jar','-sources.jar','.pom'):
        source=ROOT/'hardware/target/scala-2.13'/('zircon-asic_2.13-0.2.0'+suffix)
        shutil.copy2(source,ROOT/'dist'/source.name)
    artifacts=sorted(p for p in (ROOT/'dist').glob('*0.2.0*') if not p.name.startswith('SHA256SUMS'))
    (ROOT/'dist/SHA256SUMS-0.2.0.txt').write_text(''.join(hashlib.sha256(p.read_bytes()).hexdigest()+'  '+p.name+'\n' for p in artifacts if p.is_file()))
