...
def compute_batch(unit, a, b, c=0, *, rounding=Rounding.RNE, tags=0, backend="auto"):
    ...
    if backend != "python":
        try:
            ...
            if backend == "numba":
                raise ImportError("Numba backend requires pip install zircon-asic[fast]") from None
        else:
